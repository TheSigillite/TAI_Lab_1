...
next.addEventListener('click', function (event) {
    ...
    if (index >= preQuestions.length) {
        ...
        let prev = localStorage.getItem('oldplays');
        if(prev!=undefined){
            ...
            localStorage.setItem('oldplays',JSON.stringify({numplays: numplays,oldavg: oldavg}))
        }else{
            document.querySelector('.average').innerHTML = points;
            localStorage.setItem('oldplays',JSON.stringify({numplays: 1,oldavg: points}))
        }
    } else {
        setQuestion(index);
